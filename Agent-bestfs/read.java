...
public class read {
    ...
    public void fileOpen(){ //Input file
        try{
            x = new Scanner (new File("C:\\Users\\Areeb\\Documents\\NetBeansProjects\\CS401-K142113-A1P5\\src\\cs401\\k142113\\a1p5\\A1-tc2.txt"));
    ...
    public void fileOpenToWrite(){ //Output file
        try{
            y = new BufferedWriter (new FileWriter("C:\\Users\\Areeb\\Documents\\NetBeansProjects\\CS401-K142113-A1P5\\src\\cs401\\k142113\\a1p5\\output file.txt"));
        }
        catch(Exception e){
            System.out.println("error");
        }
    ...
    public void best_first() throws IOException{
        element[Integer.parseInt(init_1)][Integer.parseInt(init_2)].distance = 1;
        boolean path = true;
        PriorityQueue<Map.Entry<Node, Integer>> queue = new PriorityQueue<>(Comparator.comparing(entry -> entry.getValue())); //Priority queue that will pop the element with highest priority
        ...
        outerloop: while(pushed.x != Integer.parseInt(goal_1) || pushed.y != Integer.parseInt(goal_2)){
                ...
                //        System.out.println("putting in hm");
                        hm.put(pushed.neighbors[iloop], distance_to_goal);
